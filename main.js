...
function getSessions() {
  try {
    if (!fs.existsSync(sessionFile)) {
      fs.writeFileSync(sessionFile, JSON.stringify({}, null, 2), 'utf-8');
    }
    return JSON.parse(fs.readFileSync(sessionFile, 'utf-8')) || {};
  } catch (error) {
    logError(`Erreur lors de l'obtention des sessions : ${error.message}`);
    return {};
  }
}

// Changement d'assistant AI
...
// Changement de thème utilisateur
...
// Basculer le mode streamer
function toggleStreamer() {
  try {
    const updatedSettings = { ...userSettings, streamer: !userSettings.streamer };
    fs.writeFileSync(configPath, JSON.stringify(updatedSettings), 'utf-8');
    userSettings = updatedSettings;
    win.reload();
  } catch (error) {
    logError(`Erreur lors du basculement du mode streamer : ${error.message}`);
  }
}
